...
# The complete repository suite is an exact-head GitHub CI gate.  CT120 runs
# only tests that execute/validate the code participating in the physical PoC.
# This avoids unrelated repository tests becoming environment-dependent simply
# because real root-only runtime artifacts now exist on CT120.
STEP=RUNTIME_RELEVANT_UNIT_SUITE
for pattern in \
    test_executor.py \
    ...
    test_p13_wrapper_entrypoint.py; do
    if ! PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$POC_ROOT/src" \
        python3 -m unittest discover -s "$POC_ROOT/tests" -p "$pattern" >/dev/null 2>&1; then
        echo "P13_RUNTIME_RELEVANT_TEST=FAIL($pattern)"
        exit 1
    fi
done
echo "P13_RUNTIME_RELEVANT_UNIT_SUITE=PASS"
echo "P13_FULL_REPOSITORY_UNIT_SUITE_SOURCE=GITHUB_CI_EXACT_HEAD"
