...
def find_pinned_ucfg() -> Path:
    # Multiple copies of the exact same preserved UCFG are harmless. Select a
    # deterministic path, but never accept content with any other SHA-256.
    matches: list[Path] = []
    for path in base._walk_ucfg_candidates():
        try:
            if base.sha256_file(path) == base.EXPECTED_UCFG_SHA256:
                matches.append(path)
        except (OSError, PermissionError):
            continue
    if not matches:
        raise RuntimeError("pinned P12 UCFG snapshot not found")
