...
BUILD=/root/comelit-v43-transport-probe-build
RUN=/run/comelit-v43-transport-probe
OUT=/root/comelit-v43-transport-probe
...
for cmd in curl jq python3 gcc pkg-config timeout sed strings grep; do
...
STATUS_JSON="$(curl -fsS -X POST -H 'Content-Type: application/json' -d '{"action":"status"}' "$HA_URL")"
...
sed 's#^#define RUN_DIR     "/run/comelit-p2p"#' "$BASE" >/dev/null 2>&1 || true
python3 - "$BASE" "$SRC" <<'PY'
from pathlib import Path
import sys
src = Path(sys.argv[1]).read_text(encoding='utf-8')
old = '#define RUN_DIR     "/run/comelit-p2p"'
new = '#define RUN_DIR     "/run/comelit-v43-transport-probe"'
if src.count(old) != 1:
...
echo
echo '=== START SECOND PASSIVE TRANSPORT SESSION ==='
...
[ "$RC" -eq 0 ] || { echo 'CLOUD_GATE=FAIL'; exit 18; }
...
grep -E '^(ICE_COMPONENT_STATE|ICE_CONNECTED|ICE_READY|SELECTED_PAIR|PSEUDOTCP_RX_BEFORE_START|PSEUDOTCP_CONVERSATION|PSEUDOTCP_MTU|PSEUDOTCP_CONNECT_START|PSEUDOTCP_OPEN|PSEUDOTCP_CLOSED|UAUT_|UCFG_|V4_CTPP_|V4_CSPB_|V4_REGISTER|V4_RING_LISTENER_READY)' "$HELPER_LOG" | tail -n 120 || true

if [ "$TRANSPORT_READY" = true ]; then
    echo 'V43_TRANSPORT_READY=PASS'
else
    echo 'V43_TRANSPORT_READY=FAIL'
fi

echo
echo '=== STOP SECOND PASSIVE SESSION ==='
: > "$STOP"
wait "$PID" 2>/dev/null || true
PID=""

echo
echo '=== HA LISTENER AFTER ==='
STATUS_JSON="$(curl -fsS -X POST -H 'Content-Type: application/json' -d '{"action":"status"}' "$HA_URL")"
echo "$STATUS_JSON" | jq '{supervisor_running,running,listener_ready,reconnect_count,last_error}'
echo "$STATUS_JSON" | jq -e '.supervisor_running == true and .running == true and .listener_ready == true' >/dev/null || {
    echo 'HA_LISTENER_POSTCONDITION=FAIL'
    exit 19
}
echo 'HA_LISTENER_POSTCONDITION=PASS'
