...
BUILD=/root/comelit-v43-selected-pair-build
RUN=/run/comelit-v43-selected-pair
OUT=/root/comelit-v43-selected-pair
...
new_run = '#define RUN_DIR     "/run/comelit-v43-selected-pair"'
...
callback = r'''static void
new_selected_pair_cb(
    NiceAgent *nice_agent,
    guint sid,
    guint component_id,
    gchar *local_foundation,
    gchar *remote_foundation,
    gpointer data)
{
    (void)nice_agent;
    (void)local_foundation;
    (void)remote_foundation;
    (void)data;

    if (sid != stream_id || component_id != 1)
        return;

    printf("ICE_NEW_SELECTED_PAIR=true\n");

    if (!report_selected_pair()) {
        fprintf(stderr, "SELECTED_PAIR_SIGNAL_REPORT=FAIL\n");
        failed = TRUE;
        if (loop)
    ...
    if (!pseudotcp_started) {
        printf("PSEUDOTCP_START_AT_SELECTED_PAIR=true\n");
        if (!start_pseudotcp()) {
            fprintf(stderr, "PSEUDOTCP_START_AT_SELECTED_PAIR=FAIL\n");
            failed = TRUE;
            if (loop)
                g_main_loop_quit(loop);
            return;
...
if src.count(anchor) != 1:
    raise SystemExit(f'CALLBACK_PATCH=FAIL count={src.count(anchor)}')
src = src.replace(anchor, callback, 1)
...
grep -E '^(ICE_COMPONENT_STATE|ICE_CONNECTED|ICE_READY|ICE_NEW_SELECTED_PAIR|SELECTED_PAIR|PSEUDOTCP_RX_BEFORE_START|PSEUDOTCP_START_AT_SELECTED_PAIR|PSEUDOTCP_CONVERSATION|PSEUDOTCP_MTU|PSEUDOTCP_CONNECT_START|PSEUDOTCP_OPEN|PSEUDOTCP_CLOSED|UAUT_|UCFG_|V4_CTPP_|V4_CSPB_|V4_REGISTER|V4_RING_LISTENER_READY)' "$HELPER_LOG" | tail -n 180 || true

if [ "$TRANSPORT_READY" = true ]; then
    echo 'SELECTED_PAIR_TRANSPORT_READY=PASS'
else
    echo 'SELECTED_PAIR_TRANSPORT_READY=FAIL'
fi
...
echo 'SELECTED_PAIR_TRANSPORT_PROBE=PASS'
exit 0
