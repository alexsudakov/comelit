#!/usr/bin/env bash
# Offline CT122 / `hermes-node` (this host) builder for the P116/R36
# attached-media OPEN-trigger research candidate. Host identity per the
...
# none`, modeled directly on ct122_build_p116_r35_attached_media_candidate.sh,
# extended with one extra pipeline stage (the R36 overlay applied on top of
# the R35 overlay applied on top of the canonical --include-p116 source).
#
...
# R35 -- R36 never edits the R35 transform), (3) applies the NEW R36 overlay
# transform and records its digest, (4) compiles the result inside an
# Alpine 3.24.1 container with --network none using a local APK closure (no
# package index contact), and (5) runs read-only ELF/marker gates against
# the resulting candidate binary.
set -u -o pipefail
umask 077

REPO=${REPO:-/home/hermes/repos/comelit-worktrees/p116-r36-attached-media-trigger}
...
RUN_ROOT="/home/hermes/comelit-r36-build-$STAMP"
...
CANDIDATE="$RUN_ROOT/comelit-media-r36-candidate"
CONTAINER_META="$RUN_ROOT/build-meta.txt"
META="$RUN_ROOT/r36-meta-summary.txt"
FAIL=0
...
summary() {
    echo
    echo '=== COMELIT P116 R36 ATTACHED MEDIA TRIGGER CANDIDATE BUILD SUMMARY ==='
    ...
    echo "R36_GENERATED_SOURCE_SHA256=${CANDIDATE_SOURCE_SHA:-NOT_REACHED}"
    ...
    echo "R36_MARKER_GATE=${R36_MARKER_GATE:-NOT_REACHED}"
    ...
    if [ "$FAIL" -eq 0 ] && [ -n "${CANDIDATE_SHA:-}" ]; then
        echo 'R36_CANDIDATE_BUILD=PASS'
    else
        echo 'R36_CANDIDATE_BUILD=FAIL'
    fi
    echo '=== END COMELIT P116 R36 ATTACHED MEDIA TRIGGER CANDIDATE BUILD SUMMARY ==='
}
trap summary EXIT

if [ "$(id -u)" -eq 0 ]; then
    fail 'R36_BUILD_MUST_RUN_AS_NON_ROOT=true'
    exit 1
fi

for command in git python3 sha256sum awk grep sort paste stat cmp cp install docker readelf strings file; do
    command -v "$command" >/dev/null 2>&1 || fail "R36_BUILD_MISSING_COMMAND=$command"
done
[ "$FAIL" -eq 0 ] || exit 1

[ -e "$REPO/.git" ] || { fail 'R36_BUILD_REPO=ABSENT'; exit 1; }
REPO_HEAD="$(git -C "$REPO" rev-parse HEAD 2>/dev/null || true)"
echo "REPO_HEAD=$REPO_HEAD"

[ -f "$SAFETY_POC/$SOURCE_REL" ] || { fail 'R36_BUILD_SOURCE=ABSENT'; exit 1; }
[ -f "$SAFETY_POC/$CANONICAL_TRANSFORM_REL" ] || { fail 'R36_BUILD_CANONICAL_TRANSFORM=ABSENT'; exit 1; }
[ -f "$SAFETY_POC/$R35_TRANSFORM_REL" ] || { fail 'R36_BUILD_R35_TRANSFORM=ABSENT'; exit 1; }
[ -f "$SAFETY_POC/$R36_TRANSFORM_REL" ] || { fail 'R36_BUILD_R36_TRANSFORM=ABSENT'; exit 1; }
[ -d "$APK_CLOSURE" ] || { fail 'R36_BUILD_APK_CLOSURE=ABSENT'; exit 1; }
...
python3 "$SAFETY_POC/$CANONICAL_TRANSFORM_REL" \
    ...
    --output "$GENERATED" \
    --include-p116
[ -s "$GENERATED" ] || { fail 'R36_BUILD_CANONICAL_SOURCE=EMPTY'; exit 1; }
...
if [ "$CANONICAL_SHA" = "$EXPECTED_CANONICAL_SOURCE_SHA" ]; then
    ...
    fail "R36_CANONICAL_SOURCE_GATE=FAIL expected=$EXPECTED_CANONICAL_SOURCE_SHA actual=$CANONICAL_SHA"
fi
[ "$FAIL" -eq 0 ] || exit 1
echo "CANONICAL_SOURCE_GATE=$CANONICAL_SOURCE_GATE"

echo '=== R35 OVERLAY TRANSFORM (unedited by R36, reproducibility check only) ==='
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$SAFETY_POC/research/media/v1" \
...
if [ "$R35_SOURCE_SHA" = "$EXPECTED_R35_SOURCE_SHA" ]; then
    ...
    fail "R36_R35_OVERLAY_REPRODUCIBLE_GATE=FAIL expected=$EXPECTED_R35_SOURCE_SHA actual=$R35_SOURCE_SHA"
fi
[ "$FAIL" -eq 0 ] || exit 1
echo "R35_OVERLAY_REPRODUCIBLE_GATE=$R35_OVERLAY_GATE"

echo '=== R36 OVERLAY TRANSFORM (NEW this round, applied on top of R35, never edits R35) ==='
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$SAFETY_POC/research/media/v1" \
...
[ -s "$CANDIDATE_SOURCE" ] || { fail 'R36_GENERATED_SOURCE=EMPTY'; exit 1; }
CANDIDATE_SOURCE_SHA="$(sha256sum "$CANDIDATE_SOURCE" | awk '{print $1}')"
echo "R36_GENERATED_SOURCE_SHA256=$CANDIDATE_SOURCE_SHA"
...
do
    grep -Fq "$marker" "$CANDIDATE_SOURCE" || fail "R36_SOURCE_MARKER_GATE=FAIL marker=$marker"
done
[ "$FAIL" -eq 0 ] && R36_MARKER_GATE=PASS || R36_MARKER_GATE=FAIL
[ "$FAIL" -eq 0 ] || exit 1
echo "R36_MARKER_GATE=$R36_MARKER_GATE"
...
cc -O2 -g -Wall -Wextra -Wl,--as-needed -o /w/comelit-media-r36-candidate /w/comelit-media.c \
   $(pkg-config --cflags --libs nice glib-2.0 gio-2.0 gobject-2.0)
chmod 755 /w/comelit-media-r36-candidate
INTERPRETER="$(readelf -l /w/comelit-media-r36-candidate | sed -n 's@.*Requesting program interpreter: \(.*\)]@\1@p')"
NEEDED="$(readelf -d /w/comelit-media-r36-candidate | sed -n 's/.*Shared library: \[\(.*\)\]/\1/p' | sort | paste -sd, -)"
BUILDID="$(readelf -n /w/comelit-media-r36-candidate 2>/dev/null | sed -n 's/.*Build ID: \(.*\)/\1/p' | head -1)"
{
...
if [ "$BUILD_RC" -ne 0 ] || [ ! -s "$CANDIDATE" ]; then
    fail 'R36_CANDIDATE_COMPILE=FAIL'
    exit 1
fi
...
[ "$MUSL_INTERPRETER_GATE" = PASS ] || fail "R36_INTERPRETER_GATE=FAIL actual=$INTERPRETER"
[ "$NEEDED" = "$EXPECTED_NEEDED" ] && NO_NEW_RUNTIME_DEPENDENCY=PASS || NO_NEW_RUNTIME_DEPENDENCY=FAIL
[ "$NO_NEW_RUNTIME_DEPENDENCY" = PASS ] || fail "R36_NEEDED_GATE=FAIL actual=$NEEDED"
case ",$NEEDED," in
    *,libc.so.6,*) NO_GLIBC_DEPENDENCY=FAIL ;;
    *) NO_GLIBC_DEPENDENCY=PASS ;;
esac
[ "$NO_GLIBC_DEPENDENCY" = PASS ] || fail 'R36_GLIBC_DEPENDENCY=FAIL'
...
for so in $(echo "$NEEDED" | tr ',' ' '); do
    ...
    if [ ! -e "$SAFETY_POC/../custom_components/comelit/native/lib/$so" ]; then
        echo "R36_NEW_RUNTIME_DEP=$so"
        MISSING=1
    fi
done
[ "$MISSING" -eq 0 ] || { NO_NEW_RUNTIME_DEPENDENCY=FAIL; fail 'R36_PACKAGED_LIB_PRESENCE_GATE=FAIL'; }
...
for needed in libglib-2.0.so.0 libgobject-2.0.so.0 libnice.so.10; do
    ...
    if [ ! -f "$packaged" ]; then
        LIB_IDENTICAL=FAIL
        fail "R36_LIB_IDENTICAL=FAIL lib=$needed reason=packaged_absent"
    elif [ ! -f "$container" ]; then
        LIB_IDENTICAL=FAIL
        fail "R36_LIB_IDENTICAL=FAIL lib=$needed reason=container_copy_absent"
    elif ! cmp -s "$packaged" "$container"; then
        LIB_IDENTICAL=FAIL
        fail "R36_LIB_IDENTICAL=FAIL lib=$needed reason=content_mismatch"
    fi
done
...
do
    grep -Fq "$marker" "$RUN_ROOT/candidate.strings" || fail "R36_CANDIDATE_MARKER_GATE=FAIL marker=$marker"
done
[ "$FAIL" -eq 0 ] && PRODUCTION_MARKER_GATE=PASS || PRODUCTION_MARKER_GATE=FAIL
echo "R36_CANDIDATE_MARKER_GATE=$PRODUCTION_MARKER_GATE"
if grep -Fq '/lib64/ld-linux-x86-64.so.2' "$RUN_ROOT/candidate.strings"; then
    fail 'R36_CANDIDATE_GLIBC_INTERPRETER_LEAK=FAIL'
fi
[ "$FAIL" -eq 0 ] || exit 1
