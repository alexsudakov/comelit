#!/usr/bin/env bash
# Offline CT122 / `hermes-node` (this host) builder for the P116/R37
# attached-media live-readiness research candidate. Host identity per the
# private project context, PROJECT_CONTEXT.md §2.3 "CT122 -- Hermes/Codex
# orchestration node" ("Host: hermes-node / CT122") -- this is the Hermes/
# Codex orchestration node, not the CT120 Comelit research/validation node
# (§2.2). Runs as the non-root `hermes` user via `docker run --network
# none`, modeled directly on ct122_build_p116_r36_attached_media_trigger_
# candidate.sh, extended with one extra pipeline stage (the R37 overlay
# applied on top of the R36 overlay applied on top of the R35 overlay
# applied on top of the canonical --include-p116 source).
#
...
# the R35 overlay transform and checks its pinned digest (unchanged from
# R35/R36 -- R37 never edits either), (3) applies the R36 overlay transform
# and checks its pinned digest (unchanged from R36 -- R37 never edits it),
# (4) applies the NEW R37 overlay transform and records its digest, (5)
# compiles the result inside an Alpine 3.24.1 container with --network none
# using a local APK closure (no package index contact), and (6) runs
# read-only ELF/marker gates against the resulting candidate binary.
set -u -o pipefail
umask 077

REPO=${REPO:-/home/hermes/repos/comelit-worktrees/p116-r37-live-readiness}
...
R36_TRANSFORM_REL=research/media/v1/entrance_p116_r36_attached_media_trigger_transform.py
R37_TRANSFORM_REL=research/media/v1/entrance_p116_r37_attached_media_live_readiness_transform.py
...
EXPECTED_R35_SOURCE_SHA=${EXPECTED_R35_SOURCE_SHA:-5aa1662c2f75d01033c8ba6c773bffc6e8bb289a41f2e16fed417635ce1380a0}
EXPECTED_R36_SOURCE_SHA=${EXPECTED_R36_SOURCE_SHA:-59262cd3ff1ff87b2ac8612fc38e5d0c3c789e6bbad9204e5a20915c237cc1b5}
...
RUN_ROOT="/home/hermes/comelit-r37-build-$STAMP"
GENERATED="$RUN_ROOT/comelit-media-canonical.c"
R35_SOURCE="$RUN_ROOT/comelit-media-r35.c"
R36_SOURCE="$RUN_ROOT/comelit-media-r36.c"
CANDIDATE_SOURCE="$RUN_ROOT/comelit-media.c"
CANDIDATE="$RUN_ROOT/comelit-media-r37-candidate"
CONTAINER_META="$RUN_ROOT/build-meta.txt"
META="$RUN_ROOT/r37-meta-summary.txt"
FAIL=0
...
summary() {
    echo
    echo '=== COMELIT P116 R37 ATTACHED MEDIA LIVE READINESS CANDIDATE BUILD SUMMARY ==='
    ...
    echo "R36_TRANSFORM=$R36_TRANSFORM_REL"
    echo "R37_TRANSFORM=$R37_TRANSFORM_REL"
    ...
    echo "R35_GENERATED_SOURCE_SHA256=${R35_SOURCE_SHA:-NOT_REACHED}"
    echo "R35_OVERLAY_REPRODUCIBLE_GATE=${R35_OVERLAY_GATE:-NOT_REACHED}"
    echo "R36_GENERATED_SOURCE_SHA256=${R36_SOURCE_SHA:-NOT_REACHED}"
    echo "R36_OVERLAY_REPRODUCIBLE_GATE=${R36_OVERLAY_GATE:-NOT_REACHED}"
    echo "R37_GENERATED_SOURCE_SHA256=${CANDIDATE_SOURCE_SHA:-NOT_REACHED}"
    ...
    echo "R37_MARKER_GATE=${R37_MARKER_GATE:-NOT_REACHED}"
    ...
    if [ "$FAIL" -eq 0 ] && [ -n "${CANDIDATE_SHA:-}" ]; then
        echo 'R37_CANDIDATE_BUILD=PASS'
    else
        echo 'R37_CANDIDATE_BUILD=FAIL'
    fi
    echo '=== END COMELIT P116 R37 ATTACHED MEDIA LIVE READINESS CANDIDATE BUILD SUMMARY ==='
}
trap summary EXIT

if [ "$(id -u)" -eq 0 ]; then
    fail 'R37_BUILD_MUST_RUN_AS_NON_ROOT=true'
    exit 1
fi

for command in git python3 sha256sum awk grep sort paste stat cmp cp install docker readelf strings file; do
    command -v "$command" >/dev/null 2>&1 || fail "R37_BUILD_MISSING_COMMAND=$command"
done
[ "$FAIL" -eq 0 ] || exit 1

[ -e "$REPO/.git" ] || { fail 'R37_BUILD_REPO=ABSENT'; exit 1; }
REPO_HEAD="$(git -C "$REPO" rev-parse HEAD 2>/dev/null || true)"
echo "REPO_HEAD=$REPO_HEAD"

[ -f "$SAFETY_POC/$SOURCE_REL" ] || { fail 'R37_BUILD_SOURCE=ABSENT'; exit 1; }
[ -f "$SAFETY_POC/$CANONICAL_TRANSFORM_REL" ] || { fail 'R37_BUILD_CANONICAL_TRANSFORM=ABSENT'; exit 1; }
[ -f "$SAFETY_POC/$R35_TRANSFORM_REL" ] || { fail 'R37_BUILD_R35_TRANSFORM=ABSENT'; exit 1; }
[ -f "$SAFETY_POC/$R36_TRANSFORM_REL" ] || { fail 'R37_BUILD_R36_TRANSFORM=ABSENT'; exit 1; }
[ -f "$SAFETY_POC/$R37_TRANSFORM_REL" ] || { fail 'R37_BUILD_R37_TRANSFORM=ABSENT'; exit 1; }
[ -d "$APK_CLOSURE" ] || { fail 'R37_BUILD_APK_CLOSURE=ABSENT'; exit 1; }
...
python3 "$SAFETY_POC/$CANONICAL_TRANSFORM_REL" \
    ...
    --output "$GENERATED" \
    --include-p116
[ -s "$GENERATED" ] || { fail 'R37_BUILD_CANONICAL_SOURCE=EMPTY'; exit 1; }
...
if [ "$CANONICAL_SHA" = "$EXPECTED_CANONICAL_SOURCE_SHA" ]; then
    ...
    fail "R37_CANONICAL_SOURCE_GATE=FAIL expected=$EXPECTED_CANONICAL_SOURCE_SHA actual=$CANONICAL_SHA"
fi
[ "$FAIL" -eq 0 ] || exit 1
echo "CANONICAL_SOURCE_GATE=$CANONICAL_SOURCE_GATE"

echo '=== R35 OVERLAY TRANSFORM (unedited by R36/R37, reproducibility check only) ==='
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$SAFETY_POC/research/media/v1" \
...
[ -s "$R35_SOURCE" ] || { fail 'R35_GENERATED_SOURCE=EMPTY'; exit 1; }
R35_SOURCE_SHA="$(sha256sum "$R35_SOURCE" | awk '{print $1}')"
echo "R35_GENERATED_SOURCE_SHA256=$R35_SOURCE_SHA"
if [ "$R35_SOURCE_SHA" = "$EXPECTED_R35_SOURCE_SHA" ]; then
    R35_OVERLAY_GATE=PASS
else
    R35_OVERLAY_GATE=FAIL
    fail "R37_R35_OVERLAY_REPRODUCIBLE_GATE=FAIL expected=$EXPECTED_R35_SOURCE_SHA actual=$R35_SOURCE_SHA"
fi
[ "$FAIL" -eq 0 ] || exit 1
echo "R35_OVERLAY_REPRODUCIBLE_GATE=$R35_OVERLAY_GATE"

echo '=== R36 OVERLAY TRANSFORM (unedited by R37, reproducibility check only) ==='
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$SAFETY_POC/research/media/v1" \
python3 "$SAFETY_POC/$R36_TRANSFORM_REL" \
    --source "$R35_SOURCE" \
    --output "$R36_SOURCE"
R36_TRANSFORM_RC=$?
echo "R36_TRANSFORM_RC=$R36_TRANSFORM_RC"
[ "$R36_TRANSFORM_RC" -eq 0 ] || { fail 'R36_TRANSFORM=FAIL'; exit 1; }
[ -s "$R36_SOURCE" ] || { fail 'R36_GENERATED_SOURCE=EMPTY'; exit 1; }
R36_SOURCE_SHA="$(sha256sum "$R36_SOURCE" | awk '{print $1}')"
echo "R36_GENERATED_SOURCE_SHA256=$R36_SOURCE_SHA"
if [ "$R36_SOURCE_SHA" = "$EXPECTED_R36_SOURCE_SHA" ]; then
    R36_OVERLAY_GATE=PASS
else
    R36_OVERLAY_GATE=FAIL
    fail "R37_R36_OVERLAY_REPRODUCIBLE_GATE=FAIL expected=$EXPECTED_R36_SOURCE_SHA actual=$R36_SOURCE_SHA"
fi
[ "$FAIL" -eq 0 ] || exit 1
echo "R36_OVERLAY_REPRODUCIBLE_GATE=$R36_OVERLAY_GATE"

echo '=== R37 OVERLAY TRANSFORM (NEW this round, applied on top of R36, never edits R36/R35) ==='
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH="$SAFETY_POC/research/media/v1" \
python3 "$SAFETY_POC/$R37_TRANSFORM_REL" \
    --source "$R36_SOURCE" \
    --output "$CANDIDATE_SOURCE"
R37_TRANSFORM_RC=$?
echo "R37_TRANSFORM_RC=$R37_TRANSFORM_RC"
[ "$R37_TRANSFORM_RC" -eq 0 ] || { fail 'R37_TRANSFORM=FAIL'; exit 1; }
[ -s "$CANDIDATE_SOURCE" ] || { fail 'R37_GENERATED_SOURCE=EMPTY'; exit 1; }
CANDIDATE_SOURCE_SHA="$(sha256sum "$CANDIDATE_SOURCE" | awk '{print $1}')"
echo "R37_GENERATED_SOURCE_SHA256=$CANDIDATE_SOURCE_SHA"
...
for marker in \
    ...
    'r36_trigger_open_from_capabilities' \
    'r37_bounded_stop_request' \
    ...
    'R36_OP_CAPABILITIES' \
    'R37_OP_RELEASE' \
    'BOUNDED_STOP_CONTROL_KIND=SIGUSR2' \
...
do
    grep -Fq "$marker" "$CANDIDATE_SOURCE" || fail "R37_SOURCE_MARKER_GATE=FAIL marker=$marker"
done
[ "$FAIL" -eq 0 ] && R37_MARKER_GATE=PASS || R37_MARKER_GATE=FAIL
[ "$FAIL" -eq 0 ] || exit 1
echo "R37_MARKER_GATE=$R37_MARKER_GATE"
...
cc -O2 -g -Wall -Wextra -Wl,--as-needed -o /w/comelit-media-r37-candidate /w/comelit-media.c \
   $(pkg-config --cflags --libs nice glib-2.0 gio-2.0 gobject-2.0)
chmod 755 /w/comelit-media-r37-candidate
INTERPRETER="$(readelf -l /w/comelit-media-r37-candidate | sed -n 's@.*Requesting program interpreter: \(.*\)]@\1@p')"
NEEDED="$(readelf -d /w/comelit-media-r37-candidate | sed -n 's/.*Shared library: \[\(.*\)\]/\1/p' | sort | paste -sd, -)"
BUILDID="$(readelf -n /w/comelit-media-r37-candidate 2>/dev/null | sed -n 's/.*Build ID: \(.*\)/\1/p' | head -1)"
{
...
if [ "$BUILD_RC" -ne 0 ] || [ ! -s "$CANDIDATE" ]; then
    fail 'R37_CANDIDATE_COMPILE=FAIL'
    exit 1
fi
...
[ "$MUSL_INTERPRETER_GATE" = PASS ] || fail "R37_INTERPRETER_GATE=FAIL actual=$INTERPRETER"
[ "$NEEDED" = "$EXPECTED_NEEDED" ] && NO_NEW_RUNTIME_DEPENDENCY=PASS || NO_NEW_RUNTIME_DEPENDENCY=FAIL
[ "$NO_NEW_RUNTIME_DEPENDENCY" = PASS ] || fail "R37_NEEDED_GATE=FAIL actual=$NEEDED"
case ",$NEEDED," in
    *,libc.so.6,*) NO_GLIBC_DEPENDENCY=FAIL ;;
    *) NO_GLIBC_DEPENDENCY=PASS ;;
esac
[ "$NO_GLIBC_DEPENDENCY" = PASS ] || fail 'R37_GLIBC_DEPENDENCY=FAIL'
...
for so in $(echo "$NEEDED" | tr ',' ' '); do
    ...
    if [ ! -e "$SAFETY_POC/../custom_components/comelit/native/lib/$so" ]; then
        echo "R37_NEW_RUNTIME_DEP=$so"
        MISSING=1
    fi
done
[ "$MISSING" -eq 0 ] || { NO_NEW_RUNTIME_DEPENDENCY=FAIL; fail 'R37_PACKAGED_LIB_PRESENCE_GATE=FAIL'; }
...
for needed in libglib-2.0.so.0 libgobject-2.0.so.0 libnice.so.10; do
    ...
    if [ ! -f "$packaged" ]; then
        LIB_IDENTICAL=FAIL
        fail "R37_LIB_IDENTICAL=FAIL lib=$needed reason=packaged_absent"
    elif [ ! -f "$container" ]; then
        LIB_IDENTICAL=FAIL
        fail "R37_LIB_IDENTICAL=FAIL lib=$needed reason=container_copy_absent"
    elif ! cmp -s "$packaged" "$container"; then
        LIB_IDENTICAL=FAIL
        fail "R37_LIB_IDENTICAL=FAIL lib=$needed reason=content_mismatch"
    fi
done
...
for marker in \
    ...
    'MEDIA_STOP' \
    'R35_RTP_ARMED=' \
    'R36_CAPABILITIES_OBSERVED=' \
    'R36_TRIGGER_RESULT=' \
    'BOUNDED_STOP_REQUEST_RECEIVED=' \
    'CALL_BOUND_MEDIA_STOP_SENT_COUNT=' \
    'RTP_DISARMED=' \
    'MEDIA_RX_CHANNEL_DISPOSED=' \
    'R37_BOUNDED_STOP_CONTROL_KIND=SIGUSR2_UNIX_SIGNAL_SOURCE' \
    'R37_CAPABILITY_CLEARED_OBSERVED=' \
    'R37_REMOTE_RELEASE_OBSERVED=' \
    'R37_PROTOCOL_STOP_RESULT=' \
    'P80_MEDIA_ACTIVE=true' \
    'P80_VIDEO_RTP_FORWARDING=PASS' \
...
do
    grep -Fq "$marker" "$RUN_ROOT/candidate.strings" || fail "R37_CANDIDATE_MARKER_GATE=FAIL marker=$marker"
done
[ "$FAIL" -eq 0 ] && PRODUCTION_MARKER_GATE=PASS || PRODUCTION_MARKER_GATE=FAIL
echo "R37_CANDIDATE_MARKER_GATE=$PRODUCTION_MARKER_GATE"
if grep -Fq '/lib64/ld-linux-x86-64.so.2' "$RUN_ROOT/candidate.strings"; then
    fail 'R37_CANDIDATE_GLIBC_INTERPRETER_LEAK=FAIL'
fi
[ "$FAIL" -eq 0 ] || exit 1
