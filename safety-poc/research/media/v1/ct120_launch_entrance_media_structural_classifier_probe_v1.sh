#!/usr/bin/env bash
# Quiet CT120 launcher for one bounded post-0x0008 structural classification run.
#
# It reuses the exact reviewed P30 one-shot runner and selects the reviewed
# structural classifier transform. The live boundary remains:
#
...
# The patched reviewed runner is invoked exactly once and is never retried.
...
CLASSIFIER_TRANSFORM_REL=safety-poc/research/media/v1/entrance_media_structural_classifier_transform.py
...
RUN_ROOT="/root/comelit-entrance-media-structural-launch-$STAMP"
...
write_summary() {
    ...
    local struct_frames struct_ctpp struct_other struct_malformed struct_tail struct_frame_lines
    ...
    struct_frames="$(last_value 'ENTRANCE_MEDIA_STRUCT_FRAMES=' 'NOT_REACHED')"
    struct_ctpp="$(last_value 'ENTRANCE_MEDIA_STRUCT_CTPP_FRAMES=' 'NOT_REACHED')"
    struct_other="$(last_value 'ENTRANCE_MEDIA_STRUCT_OTHER_FRAMES=' 'NOT_REACHED')"
    struct_malformed="$(last_value 'ENTRANCE_MEDIA_STRUCT_MALFORMED=' 'NOT_REACHED')"
    struct_tail="$(last_value 'ENTRANCE_MEDIA_STRUCT_TAIL_BYTES=' 'NOT_REACHED')"
    struct_frame_lines="$(count_contains 'ENTRANCE_MEDIA_STRUCT_FRAME=')"
    struct_raw="$(last_value 'ENTRANCE_MEDIA_STRUCT_RAW_PAYLOAD_EMITTED=' 'NOT_REACHED')"
    struct_hex="$(last_value 'ENTRANCE_MEDIA_STRUCT_HEX_EMITTED=' 'NOT_REACHED')"
    struct_base64="$(last_value 'ENTRANCE_MEDIA_STRUCT_BASE64_EMITTED=' 'NOT_REACHED')"
    ...
    {
        echo '=== COMELIT ENTRANCE MEDIA STRUCTURAL SUMMARY ==='
        ...
        echo "STRUCT_FRAME_LINES=$struct_frame_lines"
        ...
        echo
        echo '=== STRUCTURAL FRAME METADATA ==='
        grep -F 'ENTRANCE_MEDIA_STRUCT_FRAME=' "$DETAIL_LOG" 2>/dev/null || true
        echo '=== END STRUCTURAL FRAME METADATA ==='
        echo
        echo "DETAIL_LOG=$DETAIL_LOG"
        echo "SUMMARY_FILE=$SUMMARY_FILE"
        echo "CT120_ENTRANCE_MEDIA_STRUCTURAL_LAUNCH=$final_gate"
    } > "$SUMMARY_FILE"
...
finish() {
    ...
    if ! grep -Fxq 'CT120_ENTRANCE_MEDIA_STRUCTURAL_LAUNCH=PASS' "$SUMMARY_FILE"; then
...
if [ "$CLASSIFIER_TRANSFORM_BLOB" != "$CLASSIFIER_TRANSFORM_EXPECTED_BLOB" ]; then
    echo "LAUNCHER_CLASSIFIER_TRANSFORM_BLOB_GATE=FAIL actual=$CLASSIFIER_TRANSFORM_BLOB expected=$CLASSIFIER_TRANSFORM_EXPECTED_BLOB" >> "$DETAIL_LOG"
    exit 1
fi
...
print("RUNNER_TRANSFORM=entrance_media_structural_classifier_transform.py")
...
# Exactly one runner invocation. The reviewed runner itself retains preflight
# ordering, one network wrapper invocation maximum, the 75 s outer timeout,
# no automatic retry, and mandatory listener restoration on EXIT.
