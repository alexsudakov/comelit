...
finalize_sinks() {
    ...
    SINK_FINAL_VIDEO_RTP_DATAGRAMS="$(read_sink_count "$RUN_ROOT/video.count" NOT_REACHED)"
    SINK_FINAL_AUDIO_RTP_DATAGRAMS="$(read_sink_count "$RUN_ROOT/audio.count" NOT_REACHED)"
...
main() {
    ...
    if [ -n "$CANDIDATE_PID" ] && kill -0 "$CANDIDATE_PID" 2>/dev/null; then
        ...
        PROCESS_ALIVE_AFTER_OBSERVATION=false
    fi
