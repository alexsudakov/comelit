...
SINK_FINAL_AUDIO_RTP_DATAGRAMS=NOT_REACHED
RTP_COUNTER_CONSISTENCY=NOT_COMPARABLE
RTP_OBSERVATION_SECONDS=0
RTP_OBSERVATION_DURATION_MS=UNKNOWN
...
OPEN_OBSERVED_AT_MS=NOT_OBSERVED
RUNNER_MONOTONIC_OPEN_OBSERVED_AT_MS=NOT_OBSERVED
RTP_OBSERVATION_STARTED_AT_MS=NOT_OBSERVED
RTP_OBSERVATION_ENDED_AT_MS=NOT_OBSERVED
RUNNER_MONOTONIC_RTP_OBSERVATION_STARTED_AT_MS=NOT_OBSERVED
RUNNER_MONOTONIC_RTP_OBSERVATION_ENDED_AT_MS=NOT_OBSERVED
STOP_ATTEMPT_AT_MS=NOT_ATTEMPTED
RUNNER_MONOTONIC_STOP_ATTEMPT_AT_MS=NOT_ATTEMPTED
PROCESS_RUNNING_AFTER_OPEN=UNKNOWN
PROCESS_RUNNING_AT_OBSERVATION_END=UNKNOWN
PROCESS_RUNNING_BEFORE_STOP=UNKNOWN
PROCESS_RUNNING_AFTER_STOP=UNKNOWN
PROCESS_EXIT_STATUS=UNKNOWN
LIVENESS_EVIDENCE=UNKNOWN
PROCESS_ALIVE_AFTER_OBSERVATION=UNKNOWN
PROCESS_ALIVE_BEFORE_STOP=UNKNOWN
SINKS_FINALIZED=false
VIDEO_SINK_STARTED=false
...
tmp = count_file.with_suffix(count_file.suffix + ".tmp")
...
update_liveness_evidence() {
    local terminal_reason
    terminal_reason="$(last_marker R29C_CANDIDATE_EXIT_REASON "")"
    if [ -n "$terminal_reason" ] && {
        [ "$PROCESS_RUNNING_AT_OBSERVATION_END" = true ] ||
        [ "$PROCESS_RUNNING_BEFORE_STOP" = true ] ||
        [ "$PROCESS_RUNNING_AFTER_STOP" = true ]
    }; then
        LIVENESS_EVIDENCE=CONTRADICTION
    elif [ "$PROCESS_RUNNING_AFTER_OPEN" = UNKNOWN ] ||
         [ "$PROCESS_RUNNING_AT_OBSERVATION_END" = UNKNOWN ] ||
         [ "$PROCESS_RUNNING_BEFORE_STOP" = UNKNOWN ]; then
        LIVENESS_EVIDENCE=UNKNOWN
    else
        LIVENESS_EVIDENCE=MEASURED
    fi
}
...
finalize_sinks() {
    ...
    if { [ "$CANDIDATE_REPORTED_VIDEO_RTP_PACKETS" != NOT_REACHED ] && [ "$SINK_FINAL_VIDEO_RTP_DATAGRAMS" != NOT_REACHED ]; } 2>/dev/null; then
...
classify() {
    ...
    if [ "$LIVENESS_EVIDENCE" = CONTRADICTION ]; then
        RESULT_CASE=E
...
print_final_block() {
    ...
    echo "CANDIDATE_REPORTED_VIDEO_RTP_PACKETS=$CANDIDATE_REPORTED_VIDEO_RTP_PACKETS"
    echo "VIDEO_RTP_DATAGRAMS_SINK=$VIDEO_RTP_DATAGRAMS_SINK"
    echo "AUDIO_RTP_DATAGRAMS_SINK=$AUDIO_RTP_DATAGRAMS_SINK"
    echo "SINK_FINAL_VIDEO_RTP_DATAGRAMS=$SINK_FINAL_VIDEO_RTP_DATAGRAMS"
    echo "SINK_FINAL_AUDIO_RTP_DATAGRAMS=$SINK_FINAL_AUDIO_RTP_DATAGRAMS"
    echo "RTP_COUNTER_CONSISTENCY=$RTP_COUNTER_CONSISTENCY"
    echo "RTP_OBSERVATION_SECONDS=$RTP_OBSERVATION_SECONDS"
    echo "RTP_OBSERVATION_DURATION_MS=$RTP_OBSERVATION_DURATION_MS"
    echo "OPEN_OBSERVED_AT_MS=$OPEN_OBSERVED_AT_MS"
    echo "RUNNER_MONOTONIC_OPEN_OBSERVED_AT_MS=$RUNNER_MONOTONIC_OPEN_OBSERVED_AT_MS"
    echo "RTP_OBSERVATION_STARTED_AT_MS=$RTP_OBSERVATION_STARTED_AT_MS"
    echo "RTP_OBSERVATION_ENDED_AT_MS=$RTP_OBSERVATION_ENDED_AT_MS"
    echo "RUNNER_MONOTONIC_RTP_OBSERVATION_STARTED_AT_MS=$RUNNER_MONOTONIC_RTP_OBSERVATION_STARTED_AT_MS"
    echo "RUNNER_MONOTONIC_RTP_OBSERVATION_ENDED_AT_MS=$RUNNER_MONOTONIC_RTP_OBSERVATION_ENDED_AT_MS"
    echo "STOP_ATTEMPT_AT_MS=$STOP_ATTEMPT_AT_MS"
    echo "RUNNER_MONOTONIC_STOP_ATTEMPT_AT_MS=$RUNNER_MONOTONIC_STOP_ATTEMPT_AT_MS"
    echo "PROCESS_RUNNING_AFTER_OPEN=$PROCESS_RUNNING_AFTER_OPEN"
    echo "PROCESS_RUNNING_AT_OBSERVATION_END=$PROCESS_RUNNING_AT_OBSERVATION_END"
    echo "PROCESS_RUNNING_BEFORE_STOP=$PROCESS_RUNNING_BEFORE_STOP"
    echo "PROCESS_RUNNING_AFTER_STOP=$PROCESS_RUNNING_AFTER_STOP"
    echo "PROCESS_EXIT_STATUS=$PROCESS_EXIT_STATUS"
    echo "LIVENESS_EVIDENCE=$LIVENESS_EVIDENCE"
    echo "PROCESS_ALIVE_AFTER_OBSERVATION=$PROCESS_ALIVE_AFTER_OBSERVATION"
    echo "PROCESS_ALIVE_BEFORE_STOP=$PROCESS_ALIVE_BEFORE_STOP"
    echo "VIDEO_SINK_STARTED=$VIDEO_SINK_STARTED"
    echo "VIDEO_SINK_FINALIZED=$VIDEO_SINK_FINALIZED"
    echo "VIDEO_FINAL_DATAGRAM_COUNT=$VIDEO_FINAL_DATAGRAM_COUNT"
    echo "AUDIO_SINK_STARTED=$AUDIO_SINK_STARTED"
...
main() {
    ...
    PROCESS_ALIVE_AFTER_OBSERVATION="$PROCESS_RUNNING_AT_OBSERVATION_END"
    PROCESS_ALIVE_BEFORE_STOP="$PROCESS_RUNNING_BEFORE_STOP"
    CANDIDATE_REPORTED_VIDEO_RTP_PACKETS="$(last_marker P80_VIDEO_RTP_PACKETS NOT_REACHED)"
    VIDEO_RTP_PACKETS="$CANDIDATE_REPORTED_VIDEO_RTP_PACKETS"
    FIRST_RTP_EPOCH="$(read_sink_first "$RUN_ROOT/video.first")"
    if [ "$FIRST_RTP_EPOCH" != NOT_OBSERVED ] ||
       { [ "$CANDIDATE_REPORTED_VIDEO_RTP_PACKETS" != NOT_REACHED ] && [ "$CANDIDATE_REPORTED_VIDEO_RTP_PACKETS" -gt 0 ]; } 2>/dev/null; then
...
PY
    ...
    if [ "$PROCESS_RUNNING_BEFORE_STOP" = true ]; then
    ...
    PROCESS_RUNNING_AFTER_STOP="$(process_running_state "$CANDIDATE_PID")"
