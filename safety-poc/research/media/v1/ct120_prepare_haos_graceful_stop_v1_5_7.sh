#!/usr/bin/env bash
# CT120 release builder for Comelit v1.5.7.
#
# Purpose:
# - rebuild the already-reviewed v1.5.5 graceful-stop source in an Alpine/musl
#   environment compatible with HAOS;
# - verify the produced ELF interpreter and direct NEEDED set before it can
#   replace the production native artifact;
# - freeze hashes/metadata, bump the manifest, add a release contract, commit,
#   and token-authenticated push to the existing release branch.
#
# The produced binary is NEVER executed by this script. No Comelit/HA/Door/
# self-activation/media operation is performed.
#
...
EXPECTED_DIRECT_NEEDED='libnice.so.10,libgobject-2.0.so.0,libglib-2.0.so.0,libc.musl-x86_64.so.1'
...
if [ "${EUID}" -ne 0 ]; then
    echo "V157_RELEASE_REQUIRES_ROOT=true"
    exit 1
fi

if [ -z "${RELEASE_SEED_SHA:-}" ]; then
    echo "V157_RELEASE_SEED_PRESENT=false"
    exit 1
fi

echo "V157_RELEASE_SEED_SHA=$RELEASE_SEED_SHA"

for command in git python3 sha256sum strings file awk grep sed uname; do
    if ! command -v "$command" >/dev/null 2>&1; then
        fail "V157_RELEASE_MISSING_COMMAND=$command"
    fi
done
...
if [ -z "$CONTAINER_RUNTIME" ] && command -v podman >/dev/null 2>&1; then
...
if [ -z "$CONTAINER_RUNTIME" ]; then
    ...
    echo "V157_RELEASE_CONTAINER_RUNTIME=$CONTAINER_RUNTIME"
fi
...
if [ ! -d "$REPO/.git" ]; then
    fail "V157_RELEASE_REPO_PRESENT=false"
fi
...
if [ "$FAIL" -ne 0 ]; then
    echo "V157_RELEASE_PREFLIGHT=FAIL"
    exit 1
fi
...
# The source tree is mounted read-only. Only /out is writable. The container
# may access Alpine package mirrors to install build dependencies; it receives
# no Comelit secrets and never executes the produced native program.
"$CONTAINER_RUNTIME" run --rm \
  ...
  /bin/sh -eu -c '
    ...
    NEEDED="$(readelf -d /out/comelit-v4 | sed -n "s/.*Shared library: \[\(.*\)\]/\1/p" | paste -sd, -)"
...
if [ "$BUILD_RC" -ne 0 ]; then
    fail "V157_RELEASE_ALPINE_BUILD=FAIL"
fi
...
NEEDED="$(sed -n 's/^needed=//p' "$META")"

if [ "$ALPINE_VERSION" != "3.24.1" ]; then
    fail "V157_RELEASE_ALPINE_VERSION_GATE=FAIL"
else
    echo "V157_RELEASE_ALPINE_VERSION_GATE=PASS"
fi
if [ "$LIBNICE_VERSION" != "$EXPECTED_LIBNICE" ]; then
...
if [ "$GLIB_VERSION" != "$EXPECTED_GLIB" ]; then
...
if [ "$NEEDED" != "$EXPECTED_DIRECT_NEEDED" ]; then
    fail "V157_RELEASE_NEEDED_GATE=FAIL actual=$NEEDED"
else
    echo "V157_RELEASE_NEEDED_GATE=PASS $NEEDED"
fi

file "$CANDIDATE" | sed 's#^.*: #V157_RELEASE_BINARY_FILE=#'
strings -a "$CANDIDATE" > "$BUILD/candidate.strings"
STRINGS_RC=$?
echo "V157_RELEASE_STRINGS_RC=$STRINGS_RC"
if [ "$STRINGS_RC" -ne 0 ]; then
    fail "V157_RELEASE_STRINGS=FAIL"
fi
...
if [ "$MANIFEST_RC" -ne 0 ]; then
    fail "V157_RELEASE_MANIFEST_UPDATE=FAIL"
fi
...
TEST_PARSE_RC=$?
echo "V157_RELEASE_TEST_PARSE_RC=$TEST_PARSE_RC"
if [ "$TEST_PARSE_RC" -ne 0 ]; then
    fail "V157_RELEASE_TEST_PARSE=FAIL"
fi
...
if [ "$TEST_RC" -ne 0 ]; then
    fail "V157_RELEASE_CONTRACT_TEST=FAIL"
fi
...
# The release branch is detached intentionally. Commit only the reviewed release
# artifact set, then token-authenticated push HEAD to the exact branch.
git -C "$WT" add \
  "$BINARY_REL" \
  "$MANIFEST_REL" \
  "$RELEASE_DIR_REL" \
...
if git -C "$WT" diff --cached --quiet; then
    fail "V157_RELEASE_STAGED_DIFF=EMPTY"
fi
...
if [ "$PUSH_RC" -ne 0 ]; then
    fail "V157_RELEASE_TOKEN_ONLY_PUSH=FAIL"
fi
