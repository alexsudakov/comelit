...
P112_REL=safety-poc/research/media/v1/ct120_run_p112_final_run5_runner.sh
P112="$SOURCE_ROOT/$P112_REL"

EXPECTED_P112_BASE=3e7396f552567a34ed609b3c0f1d88f44a24470c
...
[ -d "$SOURCE_ROOT/.git" ] || fail "P113_RESEARCH_REPO_PRESENT=false"
[ -f "$P112" ] || fail "P113_P112_PRESENT=false"
mkdir -p "$WORK_ROOT"
chmod 700 "$WORK_ROOT"

CURRENT_HEAD="$(git -C "$SOURCE_ROOT" rev-parse HEAD 2>/dev/null || echo UNKNOWN)"
git -C "$SOURCE_ROOT" merge-base --is-ancestor "$EXPECTED_P112_BASE" "$CURRENT_HEAD" || fail "P113_P112_BASE_ANCESTRY_GATE=FAIL"
git -C "$SOURCE_ROOT" diff --quiet "$EXPECTED_P112_BASE" "$CURRENT_HEAD" -- "$P112_REL" || fail "P113_P112_CONTENT_CHANGED=FAIL"
echo "P113_P112_BASE_ANCESTRY_GATE=PASS"
echo "P113_P112_CONTENT_GATE=PASS"
...
echo "P113_P112_BASE=$EXPECTED_P112_BASE"
