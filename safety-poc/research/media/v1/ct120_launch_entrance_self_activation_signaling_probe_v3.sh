...
#
# This launcher exists for three reasons:
# 1. correct the P30 multiline grep false-positive in the Door timer preflight;
# 2. treat the repeated `ENTRANCE_SIGNALING_DOOR_ACTION_SENT=false` invariant
#    as an at-least-once safety assertion instead of an exactly-once event;
# 3. keep verbose runner output in a file and print only a compact summary
#    at the end of the terminal output.
#
# The reviewed P30 runner remains the network-capable implementation. This
# launcher materializes that exact pinned blob, applies source-text-only
# corrections to its validation logic, executes it exactly once, stores all
# verbose output in DETAIL_LOG, and never retries.
...
if src.count(old_timer) != 1:
    raise SystemExit(f"RUNNER_TIMER_PATCH_ANCHOR_COUNT={src.count(old_timer)}")
if src.count(old_live_gate) != 1:
    raise SystemExit(f"RUNNER_LIVE_GATE_PATCH_ANCHOR_COUNT={src.count(old_live_gate)}")

patched = src.replace(old_timer, new_timer, 1)
patched = patched.replace(old_live_gate, new_live_gate, 1)

if old_timer in patched:
    raise SystemExit("RUNNER_TIMER_PATCH_OLD_PATTERN_REMAINS=true")
if patched.count(new_timer) != 1:
    raise SystemExit("RUNNER_TIMER_PATCH_NEW_PATTERN_COUNT_INVALID=true")
if old_live_gate in patched:
    raise SystemExit("RUNNER_LIVE_GATE_PATCH_OLD_PATTERN_REMAINS=true")
if patched.count(new_live_gate) != 1:
    raise SystemExit("RUNNER_LIVE_GATE_PATCH_NEW_PATTERN_COUNT_INVALID=true")
