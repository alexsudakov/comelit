...
FUNCTION=IconaBridgeClient._open_door_init
FUNCTION_LEAF=_open_door_init
FUNCTION_LINE=424
BINARY_BODY_BUILDER_CALLS=1
DOOR_MESSAGE_BUILDER_CALLS=0
WRITE_PACKET_CALLS=1
READ_RESPONSE_CALLS=2
OPEN_CHANNEL_CALLS=1
CLOSE_CHANNEL_CALLS=0
WAIT_FOR_CALLS=0
OPEN_DOOR_INIT_CALLS=0
BODY_CALL_1_LINE=450
BODY_CALL_1_COMPONENTS=2
BODY_CALL_1_REQUEST_ID_SHAPE=ATTR(channel.id)
...
FUNCTION=IconaBridgeClient.open_door
FUNCTION_LEAF=open_door
FUNCTION_LINE=464
BINARY_BODY_BUILDER_CALLS=1
DOOR_MESSAGE_BUILDER_CALLS=4
WRITE_PACKET_CALLS=5
READ_RESPONSE_CALLS=2
OPEN_CHANNEL_CALLS=0
CLOSE_CHANNEL_CALLS=1
WAIT_FOR_CALLS=2
OPEN_DOOR_INIT_CALLS=1
...
class BodyModelTests(unittest.TestCase):
    def test_parses_six_writes_in_source_order_without_payload_values(self):
        ...
        self.assertTrue(all(write.source_function.startswith("IconaBridgeClient.") for write in inventory.writes))
    ...
    def test_unqualified_method_selection_is_rejected(self):
        broken = REPORT.replace("METHOD_SELECTION=QUALIFIED_CLASS_METHOD", "METHOD_SELECTION=UNQUALIFIED")
        with self.assertRaises(ValueError):
            parse_legacy_body_shape_inventory(broken)
