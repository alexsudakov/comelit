...
V156_GOOD_SHA = "c171e7d1d342d059858f0cfca4f81dc8a07679f1d18992718bec2d6ead84db86"
...
class P28HaosMuslNativeHotfixContract(unittest.TestCase):
    ...
    def test_incompatible_v155_binary_is_archived_not_production(self):
        ...
        self.assertTrue(archived.startswith(b"\x7fELF"))
        self.assertIn(GLIBC_INTERPRETER, archived)
        self.assertNotIn(MUSL_INTERPRETER, archived)
        self.assertNotEqual(archived, current)

    def test_current_binary_preserves_frozen_v153_door_safety_contract(self):
        binary = CURRENT_BINARY.read_bytes()
        for marker in (
    ...
    def test_hotfix_build_info_freezes_exact_v156_recovery(self):
        text = (
            ROOT / "safety-poc/research/door/v1_5_6/BUILD_INFO.txt"
        ).read_text(encoding="utf-8")
        for marker in (
            "release=1.5.6",
            "current_binary_sha256=" + V156_GOOD_SHA,
    ...
    def test_v155_graceful_binary_is_preserved_as_historical_evidence(self):
        archived = ARCHIVED_V155_BINARY.read_bytes()
        self.assertIn(b"PSEUDOTCP_GRACEFUL_CLOSE_REQUESTED=true", archived)
